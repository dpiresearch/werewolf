...
# 
# This version limits the message hisotry to prevent it from overrunning the token window
# if the game goes on for a while
#
class PremiumReactiveAgent(IReactiveAgent):
    ...
    async def async_notify(self, message: ActivityMessage):
        ...
        max_length = 120
        message_content = message.content.text[:max_length]
        message_text = f"[From - {message.header.sender}| {message.header.channel}]: {message_content}"
    ...
    # async def total_characters(self):
    #     # Calculate the total number of characters in all messages
    #     return sum(len(message) for message in self.message_history)
    ...
    async def async_respond(self, message: ActivityMessage) -> ActivityResponse:
        ...
        # Calculate total number of words in message history
        total_words = sum(len(entry["content"].split()) for entry in self.message_history)
        
        # Remove oldest messages until total words are less than 7000
        while total_words > 7000:
            removed_message = self.message_history.pop(0)
            total_words -= len(removed_message["content"].split())
            logger.debug("Dropped oldest message from history to maintain word limit of 7000.")
        ...
        logger.debug(f"\nMESSAGE HISTORY LENGTH: "+str(len(self.message_history)))
        total_tokens =  sum(len(message) for message in self.message_history)
        logger.debug(f"\nTOKEN total: "+ str(total_tokens))
